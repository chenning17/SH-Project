### use this as command for program: "python make_charge_spec.py 2600_00_LED_ON_C1 2600_00_LED_OFF_C1 2600_00_LED_ON_C2 2600_00_LED_OFF_C2"
import pandas as pd
...
voltage_threshold = -2.                            # units of this are volts or time not sure
###################################################### 
...
#plt.show()
#print cross.iloc[1]['eventID']
#print cross
'''
#print cross.iloc[16]['filtered_voltage']

##################################################
#charge spectrum stuff......
...
scale2  = -dt2/resistance*1e12/1e3

q = []
q2 = []
for m in range(len(lit_volt)):
    q.append(scale*lit_volt[m])
    q2.append(scale*mask_volt[m])
    
q_ped  = scale*grouped_data_ped .filtered_voltage.sum()
q_ped2  = scale*grouped_data_ped2 .filtered_voltage.sum()
...
mu2, std2 = norm.fit(q_ped2.values)
mu_ped2  = q_ped2.mean()
std_ped2 = q_ped2.std()
threshold2 = 5*std_ped2
print "Fitted mean and standard deviation of the pedestal2    : %0.3f, %0.3f  " % (mu2, std2)
print "Calculated mean and standard deviation of the pedestal2: %0.3f, %0.3f\n" % (mu_ped2, std_ped2)
...
#plt.xscale('log')
#plt.yscale('log')
plt.xlabel('charge lit [pC]')
plt.ylabel('charge mask [pC]')
plt.title('Mask vs lit charge collected')

#calculates the pearson correlation: http://stackoverflow.com/questions/3949226/calculating-pearson-correlation-and-significance-in-python
#https://en.wikipedia.org/wiki/Pearson_product-moment_correlation_coefficient
corr = np.corrcoef(q, q2)[0, 1]
print "correlation coefficient is: ", corr
plt.text(0.1, 0.01, "correlation coefficient is: "+ str(corr))
plt.savefig('plots/' +'charge_spectrum.png')
#plt.show()
print q[1]
print q2[1]
